...
- [1. 可以返回局部变量的指针](#1-可以返回局部变量的指针)  
- [2. Go提供的两种分配原语——内建函数new和make](#2-Go提供的两种分配原语——内建函数new和make)
- [3. 复合字面](#3-复合字面)
- [4. 基础类型中数组、切片、map、chan是值类型还是引用类型](#4-基础类型中数组、切片、map、chan是值类型还是引用类型)
- [5. 初始化函数init](#5-初始化函数init)
- [6. 关于指针与值](#6-关于指针与值)
- [7. 切片、map和chan有关的内联函数](#7-切片、map和chan有关的内联函数)
- [8. append的故事](8-append的故事)
- [9. string的两种遍历](9-string的两种遍历)
...
3. 根据以上例子发现了一个奇怪的但又不奇怪的现象。不论是*T还是T都可以直接调用函数。而且在对接口赋值时接口声明部分无需声明为指针也不能声明为指针。接口赋值好后，不能取内容，虽然有些接口看上去是一个指针。  
这里不再举例  
...
1.  append会在原slice底层数组容量**够**时继续使用该数组， 这种情况是返回切片不是原切片，但是顶层数组用的是原切片的数组。
2. append会在原slice底层数组容量**不够**时更换底层数组，这种情况是返回切片不是原切片，顶层数组也不是原切片的数组。
...
- 1. 如果使用<code>for range</code>遍历，得到的元素值时utf编码的rune类型  
- 2. 如果使用传统的<code>for i := ...</code>的方式，得到的是byte类型  
...
---
2020-07-16 补充
可能在写下去，有些长了，所以就分散写了，比如 [defer的一些坑](/posts/defer的一些坑) 这样的。
...
ap := &a
fmt.Println(ap, &ap)
// fmt.Println(&c)			// 不允许获取常量指针
// fmt.Println(&(&a)) 	 	// 不允许，&a本身没有引用
```
参考： [接口类型值和接口值问题，包括nil值问题](/posts/接口类型值和接口值问题包括nil值问题)
